...
class MultiLayerLSTM(nn.Module):
    ...
    def forward(self, x, h):
        ...
        return self.linear(self.dropout(self.ln(output_hidden))), (torch.concat(new_hidden, dim=0), torch.concat(new_cell, dim=0))
...
model = MultiLayerLSTM(len(dataset.vocab), HIDDEN_SIZE, NUM_LAYERS, DROPOUT).to(device)
...
for e in range(1, EPOCHS + 1):
    ...
    if e % 2 == 0:
        print(f"\n{'=' * 50}\nSample output: \n{sample(model, dataset, 'you', HIDDEN_SIZE, 400, NUM_LAYERS, )}\n{'=' * 50}\n")
